...
application.config["MAIL_USERNAME"] = "imedbot.project@gmail.com"
application.config["MAIL_PASSWORD"] = "opahmpnetnmdcqgv"
...
client = pymongo.MongoClient(
    "mongodb+srv://XiaJiang:12345aBc@cluster0.pm6gt6w.mongodb.net/?retryWrites=true&w=majority"
)
db = client.test
...
trello_token = (
    "ATTA9240ff821b7c8af2ef43de23ef11a7695702f708e5726dbf50b1765956acde3909E08759"
)
    ...
    if filename[-3:] == "txt":
        ...
        for i in range(len(labellist)):
            ...
            for j in range(1, len(contents)):
                if (
                    len(contents[j]) == len(labellist)
                    and contents[j][i] not in res[labellist[i]]
                ):
...
def resetpassword():
    if request.method == "POST":
        ...
        if answer is not None:
            finding_result = user.find_one(
                {
                    "username": username,
                    "verification question": verification_ques,
                    "answer": answer,
                }
            )
        ...
        if finding_result is None:
            ...
            if "verification question" not in finding_result.keys():
                ...
                return {
                    "status": "success",
                    "username": username,
                    "verification question": finding_result["verification question"],
                }
            ...
            if "first name" in finding_result.keys():
                ...
                if "verification question" in finding_result.keys():
                    return {
                        "status": "success",
                        "username": username,
                        "name": finding_result["first name"],
                        "question": finding_result["verification question"],
                    }
                else:
                    return {
                        "status": "success",
...
def send_email():
    ...
    msg = Message(
        "Verification code from iMedBot",
        sender="imedbot.project@gmail.com",
        recipients=[receiver],
    )
    ...
    if request.method == "POST":
        ...
        if error != "":
            ...
            new_card = trello.create_card(
                board_list,
                label_list,
                "Error" + name + " at " + str(datetime.now()),
                error,
            )
...
def get_model_patientform():
    if request.method == "POST":
        try:
            ...
            filename = os.path.join("dataset/", dataset_name_str)
            predset, target, X_columns = modelTraining.loadandprocess(
                filename, predtype=1, scaled=False
            )
            print("232", X_columns)
            print("233", predset[0])
            print("234", target[0])
            ...
            strat_shuf = StratifiedShuffleSplit(
                ...
                def plot_shap(response):
                    ...
                    def process_after_request():
                        ...
                        def f(X):
                            ...
                            result = []
                            for item in X:
                                prob = user_training_model.predict_proba(
                                    item.reshape(1, len(predset[0]))
                                )
                                # print(prob)
                                # print(prob[0][0])
                        ...
                        if shap_check == "2":
                            ...
                            shap.plots.beeswarm(
                                shap_values, color_bar=False, show=False
                            )
                            plt.colorbar()
                        ...
                        if shap_check != "3":
                            plt.savefig(
                                img_src, bbox_inches="tight", pad_inches=0.5, dpi=dpi
                            )
                        else:
...
def get_model_parameter_exam():
    ...
    try:
        ...
        validation_auc, img_src = train_mode_parameter(
            datasetname,
            learningrate,
            batchsize,
            epochs,
...
def train_mode_parameter(
    ...
    if datasetname[-3:] == "txt":
        ...
        predset, target, X_columns = modelTraining.loadandprocess(
            filename, sep=",", predtype=1, scaled=False
        )
    cur_params = {
        "mstruct": [(50, 1)],
        "idim": [len(predset[0])],
    ...
    results, score_val, img_src = modelTraining.model_gsearch_val(
        predset, target, cur_params, nsplits, seed, scores, dpi
    )
